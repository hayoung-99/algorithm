...
def solution(str1, str2):
    str1_set = [str1[i:i+2].upper() for i in range(len(str1)-1) if ('A' <= str1[i].upper() <= 'Z' and 'A' <= str1[i+1].upper() <= 'Z')]
    str2_set = [str2[i:i+2].upper() for i in range(len(str2)-1) if ('A' <= str2[i].upper() <= 'Z' and 'A' <= str2[i+1].upper() <= 'Z')]
